...
void Maze::makeNewMaze(std::size_t size, std::size_t seed, std::size_t imperfection_factor)
{
	if (size < 3) { return; }
...
void Maze::generate_maze()
{
	...
	while (!queue.empty())
	{
		...
		case 0:
			last.second -= 2;
			map[XY_to_index(last.first, last.second)] = ' ';
			map[XY_to_index(last.first, last.second + 1)] = ' ';
		...
		case 1:
			last.first += 2;
			map[XY_to_index(last.first, last.second)] = ' ';
			map[XY_to_index(last.first - 1, last.second)] = ' ';
			queue.push_back(last);
			break;
...
int Maze::make_move(std::pair<std::size_t, std::size_t> point, int direction, char * map)
{
	...
	while (counter < 4)
	{
		...
		case 0:
			...
			{
				if (map[XY_to_index(point.first, point.second - 2)] != ' ')
				{
					return 0;
				}
			}
		...
		case 1:
			...
			{
				if (map[XY_to_index(point.first + 2, point.second)] != ' ')
				{
					return 1;
				}
			}
...
int Maze::countWallsAroundCell(std::size_t X, std::size_t Y) const
{
	...
	if (map[XY_to_index(X, Y + 1)] == '#') num_of_walls++;
	if (map[XY_to_index(X, Y - 1)] == '#') num_of_walls++;
	if (map[XY_to_index(X + 1, Y)] == '#') num_of_walls++;
	if (map[XY_to_index(X - 1, Y)] == '#') num_of_walls++;
